...
function recursivelyExpandDeepestGroups(formula) {
  ...
  // regex: contains an open paren or a closed parent
  if (!formula.match(/\(|\)/g)) {
    return formula;
  }
  ...
  var expandedParenGroups = parenGroupContent.map(expand);
  ...
  function expand(group) {
    // regex: matches open or close parens
...
function deepestGroups(formula) {
  ...
  return [...formula.matchAll(deepestGroups)].map(groupFromRgx);

  //*****************
  function groupFromRgx(group) {
    return new Group(
      group.input,
      group[0],
      group.index,
...
function reduceFinalTuples(tuples) {
  ...
  //******************************
  function processTuple(tuple, idx) {
...
/*
 * atoms are tokenized (i.e: this method is invoked) while recursivly moving through the inside all of the
